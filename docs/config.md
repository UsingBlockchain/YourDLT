...
OPTIONS
  ...
  -p, --preset=(bootstrap|testnet|mainnet|dhealth)  [default: bootstrap] the network preset
  ...
  --password=password                               A password used to encrypt and decrypt private keys in preset files
                                                    like addresses.yml and preset.yml. Bootstrap prompts for a password
                                                    by default, can be provided in the command line (--password=XXXX) or
                                                    disabled in the command line (--noPassword).

  --pullImages                                      It pulls the utility images from DockerHub when running the
                                                    configuration. It only affects alpha/dev docker images.
  ...
  --upgrade                                         It regenerates the configuration reusing the previous keys. Use this
                                                    ...
                                                    date without dropping the local data. The original preset (-t),
                                                    assembly (-a), and custom preset (-a) must be used. Backup the
...
_See code: [src/commands/config.ts](https://github.com/usingblockchain/yourdlt/blob/v0.10.18/src/commands/config.ts)_
